...
author: Simon Schwab
date: last-modified
abstract: 'An introduction to Quarto for the participants of the Good Research Practice (GRP) course in 2024.'
lang: en
format:
  html:
    toc: true
...
I round the `age` to one digit; this is good enough. Is this necessary? No, but I have nothing else to process. This data set is very tidy and clean.. This never happens in real life.
...
We will do a complete case analysis (not recommended); however, here it is fit for purpose. The analysis data set is stored in the object `titanic`.
...
Quality control is a crucial step and should be part of any data analysis pipeline. You can basically test anything. A test is like a question to the data that should be true, for example:
...
```{r}
assert(titanic$age > 0 & titanic$age < 100)
```
...
hist(titanic$age, main = "Distribution of age", xlab = "Age (years)",
...
We fit a regression model to predict the binary outcome `survival` "yes". I also set the reference levels to my pleasure.

```{r}
titanic$event = titanic$survived == "yes"
...
```
However, this table is not looking nice and is also hard to interpret. I can explain why. Therefore, I make a custom table; that is how I like to report a logistic model.
